document.addEventListener("DOMContentLoaded", () => {
  ...
  const isolateHeroA380 = () => {
    ...
    if (img) {
      img.style.display = "block"; // Natively transparent image, display directly
    }
  };
  ...
  // --- Dynamic Volumetric Parallax Clouds System ---
  const initVolumetricClouds = () => {
    const bgContainer = document.querySelector(".clouds-bg-layer");
    const fgContainer = document.querySelector(".clouds-fg-layer");
    if (!bgContainer || !fgContainer) return;

    // Distant clouds move slower (10-20% scroll factor), foreground moves moderately (35-55% scroll factor)
    const cloudConfigs = [
      { zIndex: 0, scale: 1.6, opacity: 0.25, top: "15%", left: "15%", speed: 0.12, drift: 0.25 },
      { zIndex: 0, scale: 2.2, opacity: 0.18, top: "45%", left: "68%", speed: 0.10, drift: 0.18 },
      { zIndex: 0, scale: 1.8, opacity: 0.20, top: "72%", left: "20%", speed: 0.15, drift: 0.32 },
      
      { zIndex: 1, scale: 1.1, opacity: 0.30, top: "25%", left: "82%", speed: 0.38, drift: 0.55 },
      { zIndex: 1, scale: 1.5, opacity: 0.24, top: "52%", left: "-15%", speed: 0.45, drift: 0.75 },
      { zIndex: 1, scale: 1.0, opacity: 0.35, top: "82%", left: "52%", speed: 0.52, drift: 0.90 }
    ];
    ...
    cloudConfigs.forEach(cfg => {
      ...
      el.className = `volumetric-cloud ${cfg.zIndex === 1 ? "cloud-fg" : "cloud-bg"}`;
    ...
    const animateClouds = (timestamp) => {
      ...
      const delta = (timestamp - lastTime) / 16.66;
      lastTime = timestamp;

      const scrollTop = window.scrollY;

      clouds.forEach(cloud => {
        // Horizontal drift with looping wrapper
        cloud.currentX += cloud.drift * 0.04 * delta;
        if (cloud.currentX > 115) {
          cloud.currentX = -35; // Wrap back left
        }

        // Sinusoidal vertical hover
        const floatY = Math.sin((timestamp / 1800) + cloud.floatOffset) * 10;

        // Scroll parallax translation
        const parallaxY = scrollTop * cloud.speed;

        // Apply hardware-accelerated transforms
        cloud.element.style.left = `${cloud.currentX}%`;
        cloud.element.style.transform = `translate3d(0, ${floatY - parallaxY}px, 0) scale(${cloud.scale})`;

        // Smooth opacity fading at left and right viewport edges
        if (cloud.currentX < 0) {
          const factor = Math.min((cloud.currentX + 35) / 35, 1);
          cloud.element.style.opacity = cloud.opacity * factor;
        } else if (cloud.currentX > 80) {
          const factor = Math.max((115 - cloud.currentX) / 35, 0);
          cloud.element.style.opacity = cloud.opacity * factor;
        } else {
